...
class ClientConnection extends EventEmitter {
    ...
    decryptToken() {
        ...
        if (decrypted.exp) {
            const now = Date.now();
            if (decrypted.exp < now) {
                const expiredAgo = Math.floor((now - decrypted.exp) / 1000);
                this.logger.log(LOGLEVEL.ERRORS, `Token expired ${expiredAgo} seconds ago`);
                throw new Error('Token expired');
            }

            // Log token validity info
            const validFor = Math.floor((decrypted.exp - now) / 1000);
            this.logger.log(LOGLEVEL.VERBOSE, `Token valid for ${validFor} more seconds`);
        }
